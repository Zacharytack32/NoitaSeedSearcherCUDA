...
{"--enable-unlock-gating", "-eug", cmd_enable_unlock_gating, "Enable achievement-flag gating for spells (off by default)."},
};
...
// -----------------------------
// Unlock gating CLI toggle (off by default for backward parity)
// -----------------------------
// Global toggle: when true, spell tables will be gated by achievement flags.
bool g_useUnlockGating = false;
int cmd_enable_unlock_gating(int i) {
	// No parameters; enabling this will cause GenerateSpellData to pre-filter spells
	// by required_flag using the set of unlocked achievement flags.
	g_useUnlockGating = true;
	return i;
}
