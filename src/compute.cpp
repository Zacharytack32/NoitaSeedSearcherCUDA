// ============================================================================
// Compute orchestration (workers, memory arenas, seed iteration, output)
// ----------------------------------------------------------------------------
// Responsibilities:
// - EvaluateSpan: per-worker entry that generates/filters for a seed range.
// - OutputLoop: dispatch blocks to workers, collect results, and print progress.
// - InstantiateBiomes: translate biome map chunks into device-ready scopes.
// - SearchMain: top-level routine called by main() once buffers are allocated.
// ============================================================================
...
// Per-worker function: processes a contiguous span of seeds.
// Uses a scratch MemoryArena in threadMemBlock to avoid per-seed allocations.
_compute SpanRet PLATFORM_API::EvaluateSpan(
	...
	for (int currentSeed = span.seedStart; currentSeed < span.seedStart + span.seedCount; currentSeed++) {
...
#ifdef DO_WORLDGEN
			// Worldgen phase: generate each biome; unless SEEDS_AS_TRIES, also scan spawns
		...
		// Parse the linear spawn buffer into typed pointers for filter checks
...
// Main scheduling loop. Keeps a pool of workers busy with seed blocks, adapts
// block size over time, prints progress, and streams results to output.
...
// Convert host-side biome map chunks into device-uploaded BiomeWangScope list.
...
// Top-level search routine. Uploads biome data, runs the output loop, and prints final stats.
