// ============================================================================
// World generation (Wang tiles, overlays, pixel-scene preprocessing)
// ----------------------------------------------------------------------------
// Responsibilities of this file:
// - Deterministically generate biome tilemaps using a Wang tileset per biome.
// - Validate maps (pathfinding/constraints) with a limited retry loop.
// - Optionally serialize the map to an RGB buffer (IMAGE_OUTPUT mode).
// - Preprocess pixel scene images to index spawn markers by color.
//
// Key types referenced:
// - BiomeWangScope: Bindings for a specific biome instance (map, tileset, dims).
// - GeneratedBiome: Holds tile indices referencing the selected Wang variation.
// - PixelSceneLists: Host-side tables describing scene choices per biome.
//
// Reading hints:
// - Start at GenerateMap(): it shows the seed -> rng -> tile indices pathway.
// - UploadBiomeData(): one-time preprocessing to scan pixel scene PNGs and
//   cache spawn markers for faster scanning during seed evaluation.
// ============================================================================
...
// Create the worldgen RNG state by “burning” a deterministic number of steps.
// The burn count ties map width and seed together to reproduce Noita’s order
// of random numbers used during the generate/retry loop.
_universal static WorldgenPRNG GetRNG(uint32_t world_seed, int map_w) {
	...
	if (iters > 0) {
		do {
			rng.Next();          // advance RNG without using the value
			iters -= 1;
		} while (iters != 0);
	}
	return rng;
...
// Helpers for reading/writing RGB image buffers (3 bytes per pixel)
...
// GenerateMap
// --------------------------------------------------------------------------
// Given a world seed and a biome scope (tileset + geometry), generate a valid
// Wang-tiled map by trying up to MAX_TRIES different RNG seeds (derived from
// the worldgen RNG). Validation can include pathfinding over the generated
// map so spawns are reachable, etc.
_compute GeneratedBiome GenerateMap(
	...
	int MAX_TRIES = worldSeed;         // Special debug mode: try N = worldSeed
	worldSeed = SEEDS_AS_TRIES;
#else
	constexpr int MAX_TRIES = 100;     // Normal mode: cap retries
#endif

	WorldgenPRNG rng = GetRNG(worldSeed, scope.bSec.map_w);
	//if (scope.bSec.isNightmare) rng.Next(); // Example of biome-specific tweak

	// Reserve first wang_w indices for scratch; idxs points after that
	...
	while (tries < MAX_TRIES) {
		tries++;
		// Each attempt uses a new RNG derived from the previous RNG’s next U32
		WorldgenPRNG rng2 = WorldgenPRNG(rng.NextU());
		// Fill tile index buffer using the scope’s tileset and constraints
		stbhw_generate_image(idxs, scope, scope.bSec.map_w, scope.bSec.map_h + 4, rng2);
#if 0
		// Debug: dump the Wang indices (H/V and index) to stderr
		printf("SEED %i, TRY %i\n", worldSeed, tries);
...
#endif
#ifdef SEEDS_AS_TRIES
		isValid(b, miscMem, visited);   // In this mode, always evaluate (no break)
#else
		if ((DEBUG_FLAGS & DEBUG::NO_PATHFINDING) || isValid(b, miscMem, visited))
			break;                      // Stop when the map passes constraints
#endif
	}
...
#ifdef IMAGE_OUTPUT
	// When enabled, serialize the generated map as an RGB image:
	// [u32 reserved | u32 width | u32 height | width*height*RGB bytes]
	...
	for (int y = 0; y < scope.bSec.map_h; y++) {
		for (int x = 0; x < scope.bSec.map_w; x++) {
			...
			if (visited.ptr[y * scope.bSec.map_w + x] == 2 && !false)
				c = 0xff00ffU; // highlight visited/frontier cells for debugging
...
// UploadBiomeData
// --------------------------------------------------------------------------
// For each biome’s pixel scenes, load its PNG and scan for spawn-color markers
// so we can quickly iterate scene spawns later without re-reading images.
void UploadBiomeData() {
				...
				for (int16_t y = 0; y < dims.y; y++) {
					for (int16_t x = 0; x < dims.x; x++) {
						uint32_t pix = (buf[3 * (y * dims.x + x)] << 16) + (buf[3 * (y * dims.x + x) + 1] << 8) +
									   buf[3 * (y * dims.x + x) + 2];
						// Global spawn colors (index 0) apply to all biomes
						...
						// Biome-specific colors are offset after the global list
	...
	// Upload the filled HostPixelSceneLists to device-side tables
